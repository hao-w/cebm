...
def grid_search_training(num_epochs, batch_size, latent_dim, lr, data_noise_std, sgld_noise_std, sgld_step_size, sgld_num_steps, buffer_size, buffer_percent, reg_alpha, CUDA, DEVICE):
    ...
    SAVE_VERSION = 'mnist-conjugate_sgld-latentdim=%d-lr=%.2E-data_noise_std=%.2E-sgld_noise_std=%.2E-sgld_step_size=%.2E-sgld_num_steps=%.2E-buffer_size=%d-buffer_percent=%.2f-reg_alpha=%.2E' % (latent_dim, lr, data_noise_std, sgld_noise_std, sgld_step_size, sgld_num_steps, buffer_size, buffer_percent,reg_alpha)
    ## data directory
    ...
    for i in DNSs:
        for j in SGLDNSs:
            for k in REGs:
                ...
                grid_search_training(num_epochs=150, 
                                     batch_size=100, 
                                     latent_dim=128, 
                                     lr=5e-5, 
                                     data_noise_std=i, 
