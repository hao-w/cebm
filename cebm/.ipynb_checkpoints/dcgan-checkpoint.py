...
import time
"""
training procedure for standard DC-GAN.
"""
class Train_procedure():
    def __init__(self, optimizerD, optimizerG, disc, gen, train_data, num_epochs, batch_size, device, save_version):
    ...
    def train(self):
        for epoch in range(self.num_epochs):
            ...
            metrics = {'lossD': 0.0, 'lossG': 0.0, 'Dpred_real': 0.0, 'Dpred_fake': 0.0, 'Gpred_fake': 0.0}
            for b, (images, _) in enumerate(self.train_data):                
                self.optimizerG.zero_grad()
#                 self.gen.zero_grad()
                lossG, Gpred_fake, fake_images = self.optimize_gen()
                lossG.backward()
...
#                 self.disc.zero_grad()
                ...
                lossD, Dpred_real, Dpred_fake = self.optimize_disc(images, fake_images)
                lossD.backward()
                self.optimizerD.step()
                metrics['lossD'] += lossD.detach()
                metrics['lossG'] += lossG.detach()
    ...
    def optimize_gen(self):
        noise, fake_images = self.gen.forward(self.batch_size)
        pred_fake = self.disc.binary_pred(fake_images)
...
if __name__ == "__main__":
    ...
    from sebm.models import Generator, Discriminator
...
#     parser.add_argument('--mlp_output_dim', default=288, type=int)
    ...
    parser.add_argument('--hidden_dim', default="[128,128]")
    ...
    ## training config
    parser.add_argument('--num_epochs', default=215000, type=int)
    args = parser.parse_args()
    set_seed(args.seed)
    device = torch.device('cuda:%d' % args.device)
    save_version = 'dcgan-d=%s-seed=%d-lr=%s-zd=%d-disc_act=%s-gen_act=%s-arch=%s' % (args.dataset, args.seed, args.lr, args.latent_dim, args.disc_activation, args.gen_activation, args.arch)
    ...
    disc = Discriminator(arch=args.arch,
                        im_height=im_height, 
                        im_width=im_width, 
    ...
    optimizerG = getattr(torch.optim, args.optimizer)(list(gen.parameters()), lr=args.lr,betas=(beta1, 0.999))
    print('Start training...')
    trainer = Train_procedure(optimizerD, optimizerG, disc, gen, train_data, args.num_epochs, args.batch_size, device, save_version)
    trainer.train()
