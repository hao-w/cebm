...
from sebm_mnist.gaussian_conjugacy import params_to_nats, nats_to_params, posterior_nats, unnormalized_marginal

class EBM(nn.Module):
    """
    An energy based model that maps an image x to neural sufficient statistics t(x),
    which enables the sampling from the posterior distribution p(z | x),
    wher z is some latent code.
    """
    def __init__(self, pixels_dim, hidden_dim, latents_dim, reparameterized, CUDA, DEVICE, optimize_priors=False):
        super(self.__class__, self).__init__()
        
        self.neural_ss_z = nn.Sequential(
            nn.Linear(pixels_dim, hidden_dim),
            nn.LeakyReLU(),
            nn.Linear(hidden_dim, 0.5*int(hidden_dim)),
            nn.LeakyReLU(),
            nn.Linear(0.5*int(hidden_dim), latents_dim))

#         self.neural_ss_z2 = nn.Sequential(
#             nn.Linear(pixels_dim, hidden_dim),
#             nn.LeakyReLU(),
#             nn.Linear(hidden_dim, 0.5*int(hidden_dim)),
#             nn.LeakyReLU(),
#             nn.Linear(0.5*int(hidden_dim), latents_dim))
    ...
    def forward(self, images):
        p = dict()
        neural_ss_z = self.neural_ss_z(images)
#         neural_ss_z2 = self.neural_ss_z2(images)        
        posterior_nat1, posterior_nat2 = posterior_nats(self.prior_nat1, self.prior_nat2, neural_ss_z)
        ...
        if self.reparameterized:
            ...
            posterior_samples = posterior_dist.sample()
        p['posterior_log_prob'] = posterior_dist.log_prob(posterior_samples).sum(-1)
        p['energy'] = - (neural_ss_z * posterior_samples).sum(-1)
        
        prior_mu, prior_sigma = nats_to_params(self.prior_nat1, self.prior_nat2)
        p['prior_log_factor'] = Normal(prior_mu, prior_sigma).log_prob(posterior_samples).sum(-1)
        
        p['marginal_log_prob'] = unnormalized_marginal(self.prior_nat1, self.prior_nat2, neural_ss)
        p['samples'] = posterior_samples
        return p
